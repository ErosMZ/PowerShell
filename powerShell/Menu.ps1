...
do {
    ...
    switch ($QueVaHacer){
        "A" { 
              $ruta1 = Read-Host "Dime la ruta: " 
              Get-ChildItem -Path $ruta1
             }
        "B" { 
              $ruta1 = Read-Host "Dime la ruta" 
              Get-ChildItem $ruta1 -File | Measure-Object | %{$_.Count}
             }
        "C" { 
              $ruta1 = Read-Host "Dime la ruta"
              Get-ChildItem $ruta1 -Recurse | Measure-Object | %{$_.Count}
            }
        "D" { $ruta1 = Read-Host "Dime la ruta"
             (Get-ChildItem $ruta1 -Directory |Where-Object { $_.Length -gt 1GB }).Count
            }
        "E" {
